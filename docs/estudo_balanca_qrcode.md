...
### Route: `POST /api/weigh` (in `routes/api.py`)
...
### Implementation details (`routes/api.py`)
- Authenticated by `X-API-Key`, compared **timing-safe** against the per-integration keys
  (`db.verify_api_key`). The station uses the **scale** key (scope `write`), managed in
  **Admin → Integrations** and seeded at install from `SPOOL_API_KEY`. **No valid key → 401**
  — closed by default, there is no open-by-default mode.
